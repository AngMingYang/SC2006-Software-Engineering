...
class Web_Server_Backend:
    ...
    #return render_template("main menu/create_account.html")
        ...
        # Commuter SSE route that pushes updates to the client
        @self.app.route("/commuter/should_refresh", methods=["GET"])
        @login_required
        def should_refresh():
            user = UserFactory.create_user(session["username"])

            # Ensure only commuters can access this
            if user.get_type() != "Commuter":
                return jsonify({"refresh": False})

            # Return the current value of the refresh flag (which was set by the admin)
            return jsonify({"refresh": self.refresh_flag})
        ...
        @self.app.route('/lta/should_refresh')
        @login_required
        def lta_should_refresh():
            user = UserFactory.create_user(session["username"])
            if user.get_type() != "LTA_Manager":
                return jsonify({"refresh": False})

            refresh_now = self.refresh_flag
            if self.refresh_flag:
                self.refresh_flag = False  # Reset after use
            return jsonify({"refresh": refresh_now})
